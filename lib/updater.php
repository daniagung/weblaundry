...
function check_for_detheme_update(){
	...
	foreach ( $installed_themes as $theme ) {
		...
		if(preg_match("/(detheme.com)/i", $theme->get('AuthorURI')) && ''!=get_option("cleanco_license_".$theme->get_template())) {
			...
			$themes[ $theme->get_stylesheet() ] = array(
				...
				'License'    => get_option("cleanco_license_".$theme->get_template()),
			);
		}
	}
...
function check_for_detheme_plugins_update(){
	...
	$sn=get_option("cleanco_license_".$tm->get_template());
	if(empty($sn))
		return false;
...
if (detheme_plugin_is_active('revslider/revslider.php')) {

...
class detheme_icon{
	...
	function init(){
   	...
   	function load_front_css_style(){
   		...
   		if(count($icontypes)){
   			...
   			foreach ($icontypes as $name => $icontype) {
  				...
  				if( (bool)get_cleanco_option("detheme-icon-".$name,true) && ''!=$icontype['css'])
					print "@import url('". trailingslashit(untrailingslashit(detheme_maybe_ssl_url($icontype['uri']))).$icontype['css'] ."');\n";
   			}
   			print "</style>\n";
   		}
   	}
   	...
   	function get_icofont_list($icons){
   		...
   		if(count($icontypes)){
   			$icons=array();
   			foreach ($icontypes as $name => $icontype) {
   				if( get_cleanco_option("detheme-icon-".$name) && ''!=$icontype['css'] && $newicons=dt_exctract_DTicon($icontype['path']."/".$icontype['css'])){
	...
	function add_detheme_icon_section($sections){
		...
		$sections['icon-font'] = array(
			'icon' => 'el-icon-magic',
			'title' => esc_html__('Icon Settings', 'redux-framework'),
			'fields' => $fields
		);

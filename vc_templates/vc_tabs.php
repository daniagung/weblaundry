...
foreach ( $matches as $tab ) {
	$tab_stts=wp_parse_args(shortcode_parse_atts($tab[3]),array('title'=>'','tab_id'=>'','tab_icon'=>''));
    $tabs_nav .= '<li><a href="#tab-'. (''!=$tab_stts['tab_id'] ? $tab_stts['tab_id'] : sanitize_title( $tab_stts['title']) ) .'">' .
    ($tab_stts['tab_icon']!=''?"<i class=\"".$tab_stts['tab_icon']."\"></i>":"").
    $tab_stts['title'] . '</a></li>';
}
endif;

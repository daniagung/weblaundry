...
<form class="shipping_calculator" action="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" method="post">
	...
	<h2><a href="#" class="shipping-calculator-button primary_color_button"><?php _e( 'Calculate Shipping', 'woocommerce' ); ?></a></h2>
			...
			<select name="calc_shipping_country" id="calc_shipping_country" class="country_to_state" rel="calc_shipping_state">
				...
				<?php
					foreach( WC()->countries->get_shipping_countries() as $key => $value )
						echo '<option value="' . esc_attr( $key ) . '"' . selected( WC()->customer->get_shipping_country(), esc_attr( $key ), false ) . '>' . $value . '</option>';
				?>
			</select>
		</p>
